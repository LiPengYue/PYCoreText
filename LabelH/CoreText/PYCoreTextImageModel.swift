...
import UIKit
protocol PYCoreTextImageBaseModelDelegate {
    
    /// 创建string 中的image 模型
    ///
    /// - Parameter model: 网络数据模型
    /// - Returns: 返回数据模型
    func createModel<T>(model:T) -> PYCoreTextImageBaseModel
}
...
class PYCoreTextImageBaseModel: NSObject {
    var url: String?
    var frame: CGRect = .zero// { get { return framePrivate} }
    ...
    private var framePrivate = CGRect.zero
}
